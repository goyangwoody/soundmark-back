...
- **지도 API**: 지도 중심(lat/lng) 기준 10km 내 추천 표시, 사용자 위치(my_lat/my_lng) 기준 200m 이내 활성화
...
#### `GET /map/nearby`
추천곡 지도 데이터 조회 (지도 중심 10km 반경)
- **인증 필요**: ✅
- **파라미터**:
  - `my_lat` (query, 사용자 실제 위도, required) — 거리/활성 판단 기준
  - `my_lng` (query, 사용자 실제 경도, required) — 거리/활성 판단 기준
  - `lat` (query, 지도 중심 위도, required) — 추천 검색 기준
  - `lng` (query, 지도 중심 경도, required) — 추천 검색 기준
- **응답**: `MapResponse`
  ```json
  {
    "recommendations": [
  ...
  ```
- **로직**:
  1. `lat/lng` (지도 중심) 기준 **10km 반경** 내 모든 추천 검색
  2. `my_lat/my_lng` (사용자 위치) 기준 **distance_meters** 계산
  3. **is_active**: 사용자 위치에서 200m 이내면 `true`, 초과면 `false`
  4. **total_reactions**: 모든 이모지 반응의 합계
  5. PostGIS `ST_DWithin`, `ST_Distance` 사용
...
| `get_map_data(db, center_lat, center_lng, my_lat, my_lng, radius_meters=2000)` | 지도 데이터 (center 기준 검색, my 기준 거리 계산) |
